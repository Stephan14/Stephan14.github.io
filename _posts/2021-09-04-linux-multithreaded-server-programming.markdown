...
#### pipe的优缺点
1. pipe在**收发字节流**方面，是**单向**的
2. 进程双向通信还得开2个文件描述符，并且需要有父子进程才能使用
3. pipe经典的应用场景是用来异步唤醒select调用

#### TCP的优缺点
1. TCP通信可以跨机器，提高了scalability
2. port是独占的，可以防止程序重复启动
3. 可以通过tcpdump和wireshark来重现和记录
4. 可以跨语言通信
5. TCP是字节流协议，只能顺序读取，有写缓冲；共享内存需要a进程填好一块内存让b进程来读，基本是“停等”方式（需要考虑如果数据写到一半怎么办）
...
1. 一个多线程的进程和多个相同的单线程进程怎么取舍？
如果一次请求所访问的内存比较大，那么就使用多线程，避免CPU cache换入换出，影响性能
2. 还有那些多线程的编程模型
Proactor模型可以提高吞吐，但是不能降低延迟
3. 多线程程序如何让IO和"计算"相互重叠，降低latency？
